...
from typing import Any, Generic, SupportsFloat, TypeVar, get_args, get_origin, overload
...
class DataclassAdapter(Generic[T]):
    ...
    def _validate_model(self, value: dict[str, Any]) -> T | None:
        """Validate and deserialize a dict into the configured dataclass.

        This method handles both single dataclasses and list dataclasses. For list models, it expects
        the value to contain an "items" key with the list data, following the convention used by
        `_serialize_model`. If validation fails and `raise_on_validation_error` is False, returns None
        instead of raising.

        Args:
            value: The dict to validate and convert to a dataclass.

        Returns:
            The validated dataclass instance, or None if validation fails and errors are suppressed.
        ...
        try:
            ...
            if self._raise_on_validation_error:
                msg = f"Invalid dataclass: {value}"
                raise DeserializationError(msg) from e
            return None

    def _serialize_model(self, value: T) -> dict[str, Any]:
        """Serialize a dataclass to a dict for storage.

        This method handles both single dataclasses and list dataclasses. For list models, it wraps
        the serialized list in a dict with an "items" key (e.g., {"items": [...]}) to ensure consistent
        dict-based storage format across all value types. This wrapping convention is expected by
        `_validate_model` during deserialization.

        Args:
            value: The dataclass instance to serialize.

        Returns:
            A dict representation of the dataclass suitable for storage.

        Raises:
            SerializationError: If the dataclass cannot be serialized.
        """
        try:
            if self._is_list_model:
    ...
    async def get(self, key: str, *, collection: str | None = None, default: T | None = None) -> T | None:
        """Get and validate a dataclass by key.
        ...
        Returns:
            The parsed dataclass instance if found and valid, or the default value if key doesn't exist
            or validation fails.

        Raises:
            DeserializationError: If the stored data cannot be validated as the dataclass and the
            DataclassAdapter is configured to raise on validation error.
        """
    ...
    async def get_many(self, keys: Sequence[str], *, collection: str | None = None, default: T | None = None) -> list[T] | list[T | None]:
        """Batch get and validate dataclasses by keys, preserving order.
        ...
        Returns:
            A list of parsed dataclass instances, with default values for missing keys or validation failures.
    ...
    async def put(self, key: str, value: T, *, collection: str | None = None, ttl: SupportsFloat | None = None) -> None:
        """Serialize and store a dataclass.

        Propagates SerializationError if the dataclass cannot be serialized.
        """
        collection = collection or self._default_collection
    ...
    async def put_many(
        ...
        """Serialize and store multiple dataclasses, preserving order alignment with keys."""
        collection = collection or self._default_collection
    ...
    async def delete(self, key: str, *, collection: str | None = None) -> bool:
        """Delete a dataclass by key. Returns True if a value was deleted, else False."""
        collection = collection or self._default_collection

        return await self._key_value.delete(key=key, collection=collection)

    async def delete_many(self, keys: Sequence[str], *, collection: str | None = None) -> int:
        """Delete multiple dataclasses by key. Returns the count of deleted entries."""
        collection = collection or self._default_collection
    ...
    async def ttl(self, key: str, *, collection: str | None = None) -> tuple[T | None, float | None]:
        """Get a dataclass and its TTL seconds if present.

        Returns (dataclass, ttl_seconds) or (None, None) if missing.
        """
        collection = collection or self._default_collection
    ...
    async def ttl_many(self, keys: Sequence[str], *, collection: str | None = None) -> list[tuple[T | None, float | None]]:
        """Batch get dataclasses with TTLs. Each element is (dataclass|None, ttl_seconds|None)."""
        collection = collection or self._default_collection
