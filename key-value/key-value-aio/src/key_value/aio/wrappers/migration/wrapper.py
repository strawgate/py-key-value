...
class MigrationWrapper(BaseWrapper):
    ...
    def __init__(
        self,
        current_store: AsyncKeyValue,
        legacy_store: AsyncKeyValue,
    ...
    async def get(self, key: str, *, collection: str | None = None) -> dict[str, Any] | None:
        ...
        if cached_location == "current":
            ...
            return await self.current_store.get(key=key, collection=collection)
        if cached_location == "missing":
            # We know it doesn't exist
            return None

        # Try current store first
        result = await self.current_store.get(key=key, collection=collection)
        ...
        result = await self.legacy_store.get(key=key, collection=collection)
        ...
        if self.migrate_on_read:
            ...
            await self.current_store.put(key=key, value=result, collection=collection, ttl=ttl)
    ...
    async def get_many(self, keys: Sequence[str], *, collection: str | None = None) -> list[dict[str, Any] | None]:  # noqa: PLR0912, PLR0915
        ...
        if current_keys:
            current_results = await self.current_store.get_many(keys=current_keys, collection=collection)
    ...
    async def ttl(self, key: str, *, collection: str | None = None) -> tuple[dict[str, Any] | None, float | None]:
        ...
        if cached_location == "current":
            return await self.current_store.ttl(key=key, collection=collection)
        if cached_location == "missing":
            return (None, None)

        # Try current store first
        result, ttl = await self.current_store.ttl(key=key, collection=collection)
        ...
        # Fall back to legacy store
        result, ttl = await self.legacy_store.ttl(key=key, collection=collection)
    ...
    @override
    async def ttl_many(self, keys: Sequence[str], *, collection: str | None = None) -> list[tuple[dict[str, Any] | None, float | None]]:  # noqa: PLR0912
        ...
        # Fetch from current
        if current_keys:
            current_results = await self.current_store.ttl_many(keys=current_keys, collection=collection)
            for i, key in enumerate(current_keys):
                key_to_value[key] = current_results[i]

        # Fetch from legacy
        if legacy_keys:
            legacy_results = await self.legacy_store.ttl_many(keys=legacy_keys, collection=collection)
            for i, key in enumerate(legacy_keys):
                key_to_value[key] = legacy_results[i]

                if self.migrate_on_read and legacy_results[i][0] is not None:
                    value, ttl = legacy_results[i]
                    await self.current_store.put(key=key, value=value, collection=collection, ttl=ttl)  # type: ignore
                    if self.delete_after_migration:
                        await self.legacy_store.delete(key=key, collection=collection)
                    self._cache_put(key, collection, "current")
        ...
        if unknown_keys:
            ...
            if not_in_current:
                legacy_results = await self.legacy_store.ttl_many(keys=not_in_current, collection=collection)
                for i, key in enumerate(not_in_current):
    ...
    async def put(self, key: str, value: Mapping[str, Any], *, collection: str | None = None, ttl: SupportsFloat | None = None) -> None:
        ...
        await self.current_store.put(key=key, value=value, collection=collection, ttl=ttl)
    ...
    async def put_many(
        ...
        *,
        ...
        Args:
            ...
            ttl: Time-to-live in seconds.
        """
        await self.current_store.put_many(keys=keys, values=values, collection=collection, ttl=ttl)
    ...
    async def delete(self, key: str, *, collection: str | None = None) -> bool:
        ...
        current_deleted = await self.current_store.delete(key=key, collection=collection)
        legacy_deleted = await self.legacy_store.delete(key=key, collection=collection)
    ...
    async def delete_many(self, keys: Sequence[str], *, collection: str | None = None) -> int:
        ...
        current_count = await self.current_store.delete_many(keys=keys, collection=collection)
        legacy_count = await self.legacy_store.delete_many(keys=keys, collection=collection)
