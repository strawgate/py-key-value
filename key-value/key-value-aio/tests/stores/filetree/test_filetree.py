...
class TestFileTreeStore(BaseStoreTests):
    ...
    async def test_visual_inspection_collection_as_directory(self, store: FileTreeStore):
    ...
    async def test_visual_inspection_key_as_filename(self, store: FileTreeStore):
    ...
    async def test_visual_inspection_json_file_content(self, store: FileTreeStore):
        """Test that file contents are valid JSON."""
        await store.put(collection="test_collection", key="test_key", value={"test": "test_value", "number": 42})
    ...
    async def test_expired_file_remains_on_disk(self, store: FileTreeStore):
        """Test that expired entries remain on disk but are filtered on read."""
        await store.put(collection="test_collection", key="test_key", value={"test": "test"}, ttl=0.1)
    ...
    async def test_collection_enumeration(self, store: FileTreeStore):
        """Test that we can enumerate collections."""
        await store.put(collection="collection_1", key="key_1", value={"test": "1"})
        await store.put(collection="collection_2", key="key_2", value={"test": "2"})

        collections = await store.collections()
        assert "collection_1" in collections
        assert "collection_2" in collections

    async def test_key_enumeration(self, store: FileTreeStore):
        """Test that we can enumerate keys in a collection."""
        await store.put(collection="test_collection", key="key_1", value={"test": "1"})
        await store.put(collection="test_collection", key="key_2", value={"test": "2"})
        await store.put(collection="test_collection", key="key_3", value={"test": "3"})

        keys = await store.keys(collection="test_collection")
        assert "key_1" in keys
        assert "key_2" in keys
        assert "key_3" in keys

    async def test_delete_collection(self, store: FileTreeStore):
        ...
        result = await store.destroy_collection(collection="test_collection")
        assert result is True
        assert not collection_path.exists()

    async def test_delete_store(self, store: FileTreeStore):
        """Test that we can delete the entire store."""
        await store.put(collection="collection_1", key="key_1", value={"test": "1"})
        await store.put(collection="collection_2", key="key_2", value={"test": "2"})

        result = await store.destroy()
        assert result is True

        # All collections should be gone
        assert len(list(store._directory.iterdir())) == 0  # pyright: ignore[reportPrivateUsage]

    async def test_handles_missing_collection_gracefully(self, store: FileTreeStore):
        """Test that operations on missing collections are handled gracefully."""
        keys = await store.keys(collection="nonexistent_collection")
        assert keys == []

    async def test_handles_corrupted_file(self, store: FileTreeStore):
        ...
        result = await store.get(collection="test_collection", key="corrupted_key")
        assert result is None

    async def test_multiple_collections(self, store: FileTreeStore):
        """Test that multiple collections work independently."""
        await store.put(collection="collection_1", key="shared_key", value={"collection": "1"})
        await store.put(collection="collection_2", key="shared_key", value={"collection": "2"})

        result_1 = await store.get(collection="collection_1", key="shared_key")
        result_2 = await store.get(collection="collection_2", key="shared_key")

        assert result_1 == {"collection": "1"}
        assert result_2 == {"collection": "2"}

    async def test_directory_structure(self, store: FileTreeStore):
        """Test the overall directory structure is correct."""
        await store.put(collection="col1", key="key1", value={"data": "1"})
        await store.put(collection="col1", key="key2", value={"data": "2"})
        await store.put(collection="col2", key="key3", value={"data": "3"})
