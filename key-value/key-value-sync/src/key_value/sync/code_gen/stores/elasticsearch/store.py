...
DEFAULT_MAPPING = {
    "properties": {
        ...
        "key": {"type": "keyword"},
        "value": {"type": "keyword", "index": False, "doc_values": False, "ignore_above": 256},
    }
}
...
class ElasticsearchStore(
    ...
    @overload
    def __init__(self, *, elasticsearch_client: Elasticsearch, index: str, default_collection: str | None = None) -> None: ...
    ...
    def _get_managed_entry(self, *, key: str, collection: str) -> ManagedEntry | None:
        ...
        elasticsearch_response = self._client.options(ignore_status=404).get(index=self._index, id=self.sanitize_document_id(key=combo_key))
    ...
    def _get_collection_keys(self, *, collection: str, limit: int | None = None) -> list[str]:
        ...
        result: ObjectApiResponse[Any] = self._client.options(ignore_status=404).search(
            index=self._index, fields=[{"key": None}], body={"query": {"term": {"collection": collection}}}, source_includes=[], size=limit
        )
    ...
    def _get_collection_names(self, *, limit: int | None = None) -> list[str]:
        ...
        search_response: ObjectApiResponse[Any] = self._client.options(ignore_status=404).search(
            index=self._index, aggregations={"collections": {"terms": {"field": "collection"}}}, size=limit
        )
    ...
    def _delete_collection(self, *, collection: str) -> bool:
        result: ObjectApiResponse[Any] = self._client.options(ignore_status=404).delete_by_query(
            index=self._index, body={"query": {"term": {"collection": collection}}}
        )
    ...
    def _cull(self) -> None:
        _ = self._client.options(ignore_status=404).delete_by_query(
            index=self._index, body={"query": {"range": {"expires_at": {"lt": now_as_epoch()}}}}
        )
