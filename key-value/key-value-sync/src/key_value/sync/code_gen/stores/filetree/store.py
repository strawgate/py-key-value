...
class FileTreeV1CollectionSanitizationStrategy(HybridSanitizationStrategy):
    ...
    def __init__(self, directory: Path | AsyncPath) -> None:
        ...
        super().__init__(
            replacement_character="_", max_length=max_name_length - suffix_length, allowed_characters=DIRECTORY_ALLOWED_CHARACTERS
        )
...
class FileTreeV1KeySanitizationStrategy(HybridSanitizationStrategy):
    ...
    def __init__(self, directory: Path | AsyncPath) -> None:
        ...
        super().__init__(replacement_character="_", max_length=max_length, allowed_characters=FILE_NAME_ALLOWED_CHARACTERS)
...
class DiskCollectionInfo:
    ...
    def from_file(
        ...
        if data := read_file(file=file):
            resolved_directory = AsyncPath(data["directory"]).resolve()
    ...
    def _load_collection_infos(self) -> None:
        for entry in self._get_metadata_entries():
            collection_info: DiskCollectionInfo = DiskCollectionInfo.from_file(
                file=entry, serialization_adapter=self._serialization_adapter, key_sanitization_strategy=self._key_sanitization_strategy
            )
