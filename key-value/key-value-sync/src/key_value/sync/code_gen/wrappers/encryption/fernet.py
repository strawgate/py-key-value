...
class FernetEncryptionWrapper(BaseEncryptionWrapper):
    ...
    @overload
    def __init__(self, key_value: KeyValue, *, fernet: Fernet | MultiFernet, raise_on_decryption_error: bool = True) -> None:
...
def _generate_encryption_key(source_material: str, salt: str) -> bytes:
    ...
    pbkdf2 = PBKDF2HMAC(algorithm=hashes.SHA256(), length=32, salt=salt.encode(), iterations=KDF_ITERATIONS).derive(
