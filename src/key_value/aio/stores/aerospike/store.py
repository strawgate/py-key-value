...
try:
    import aerospike  # pyright: ignore]
...
def _create_aerospike_client(config: dict[str, Any]) -> aerospike.Client:  # pyright: ignore]
    """Create an Aerospike client."""
    return aerospike.client(config)  # pyright: ignore[reportUnknownMemberType]


def _connect_aerospike_client(client: aerospike.Client) -> None:  # pyright: ignore]
    """Connect the Aerospike client."""
    client.connect()  # pyright: ignore]


def _get_aerospike_record(
    client: aerospike.Client,  # pyright: ignore]
...
def _scan_aerospike_set(
    ...
    namespace: str,
    set_name: str,
    callback: Any,  # pyright: ignore]
) -> None:
    """Scan the entire set with a callback function."""
    scan = client.scan(namespace, set_name)  # pyright: ignore]
...
def _close_aerospike_client(client: aerospike.Client) -> None:  # pyright: ignore]
    """Close the Aerospike client connection."""
    client.close()  # pyright: ignore]


def _get_aerospike_namespaces(client: aerospike.Client) -> list[str]:  # pyright: ignore]
    ...
    for error_code, response in info_response.values():  # pyright: ignore]
        if error_code == 0 and response:  # pyright: ignore]
            namespaces.update(response.split(";"))  # pyright: ignore]
...
class AerospikeStore(BaseDestroyStore, BaseEnumerateKeysStore, BaseContextManagerStore, BaseStore):
    ...
    _client: aerospike.Client  # pyright: ignore]
    _namespace: str
    _set: str
    _auto_create: bool
    ...
    def __init__(
        self,
        *,
        client: aerospike.Client | None = None,  # pyright: ignore]
        hosts: list[tuple[str, int]] | None = None,
        namespace: str = DEFAULT_NAMESPACE,
        set_name: str = DEFAULT_SET,
        default_collection: str | None = None,
        ...
        if client:
            ...
            config = {"hosts": hosts}
            self._client = _create_aerospike_client(config)  # pyright: ignore]
