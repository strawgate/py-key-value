...
def _create_dynamodb_client_context(session: Session, *, endpoint_url: str | None = None) -> Any:  # pyright: ignore]
...
async def _describe_dynamodb_table(client: DynamoDBClient, table_name: str) -> bool:
    ...
    """
    try:
        await client.describe_table(TableName=table_name)  # pyright: ignore]
    except client.exceptions.ResourceNotFoundException:  # pyright: ignore]
        return False
    else:
        return True
...
async def _create_dynamodb_table(client: DynamoDBClient, table_name: str, *, table_config: dict[str, Any] | None = None) -> None:
    ...
    await client.create_table(**create_table_params)  # pyright: ignore]

    # Wait for table to be active
    waiter = client.get_waiter("table_exists")  # pyright: ignore]
    await waiter.wait(TableName=table_name)  # pyright: ignore]
...
async def _describe_dynamodb_ttl(client: DynamoDBClient, table_name: str) -> str | None:
    ...
    """
    ttl_response = await client.describe_time_to_live(  # pyright: ignore]
        TableName=table_name
    )
    return ttl_response.get("TimeToLiveDescription", {}).get("TimeToLiveStatus")  # pyright: ignore]
...
async def _enable_dynamodb_ttl(client: DynamoDBClient, table_name: str, attribute_name: str = "ttl") -> None:
    ...
    await client.update_time_to_live(  # pyright: ignore]
        TableName=table_name,
...
async def _put_dynamodb_item(client: DynamoDBClient, table_name: str, item: dict[str, Any]) -> None:
    ...
    await client.put_item(  # pyright: ignore]
        TableName=table_name,
        Item=item,
    )
...
async def _delete_dynamodb_item(client: DynamoDBClient, table_name: str, key: dict[str, Any]) -> dict[str, Any] | None:
    ...
    """
    response = await client.delete_item(  # pyright: ignore]
        TableName=table_name,
        Key=key,
        ReturnValues="ALL_OLD",
    )
    return response.get("Attributes")  # pyright: ignore]
...
class DynamoDBStore(BaseContextManagerStore, BaseStore):
    ...
    _session: aioboto3.Session  # pyright: ignore]
    _table_name: str
